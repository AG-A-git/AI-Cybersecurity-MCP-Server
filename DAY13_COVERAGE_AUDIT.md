...
| -- | ----------------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------- |
| 1  | SQL Injection                 | Existing | `scanner/rules/sql.py` contains SQL injection detection logic for SQL queries and variable concatenation.                                             | SQL test files in `scanner/tests`, `scanner/test_files`, and `scanner/test_samples` |
| 2  | Cross-Site Scripting (XSS)    | Existing | `scanner/rules/xss.py` contains XSS detection logic.                                                                                                  | XSS test files in `scanner/test_files` and `scanner/test_samples`                   |
| 3  | Command Injection             | Existing | `scanner/rules/command.py` detects dangerous command execution such as `os.system` and `subprocess` calls.                                            | `command_test.py`, `command_constant_test.py`, `command_safe_test.py`               |
| 4  | LDAP Injection                | Existing | `scanner/rules/ldap.py` detects variable concatenation into LDAP filters and creates CWE-90 findings.                                                 | `ldap_test.py` and `ldap_safe_test.py`                                              |
...
| 7  | Broken Access Control         | Missing  | `scanner/rules/access_control.py` is empty and has no meaningful detection logic.                                                                     | No meaningful tests identified                                                      |
...
* `ldap_test.py` contains a vulnerable example using user-controlled `username` in an LDAP filter.
* `ldap_safe_test.py` contains a safe LDAP filter example.
...
* `__pycache__` and compiled files were not considered as source-code evidence.
